...
class ScotlandYardAdmin extends React.Component {
  ...
  handleCellClick = (i, j) => {
    const newColor = window.confirm("Choose color:\nPress OK for 'red', Cancel for 'blue'") 
      ? 'red' : 'blue';
  
    const finalColor = newColor === 'blue' ? 'green' : newColor;
        ...
        <div className="flex flex-col flex-1">
          {gridData.slice(0, 3).map((color, index) => (
            <div
              ...
              onClick={() => this.handleCellClick(gridIndex, index)} // Handle cell click
            >
              Cell {index + 1}
            </div>
          ))}
