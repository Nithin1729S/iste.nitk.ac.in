...
def index(request):
    ...
    color_counts = [[{'red': 0, 'green': 0, 'blue': 0} for _ in range(4)] for _ in range(3)]  # Initialize color counts
    ...
    for cell in colors:
        ...
        if 0 <= cell.row < 3 and 0 <= cell.col < 4:
            ...
            if cell.color == "red":
                color_counts[cell.row][cell.col]['red'] += 1
            elif cell.color == "green":
                color_counts[cell.row][cell.col]['green'] += 1
            elif cell.color == "blue":
                color_counts[cell.row][cell.col]['blue'] += 1
        else:
            print(f"Invalid cell coordinates: ({cell.row}, {cell.col})")  # Log any out-of-bound cells

    # Return the grid colors and counts as JSON
    return JsonResponse({'grid_colors': grid_colors, 'counts': color_counts})
...
def update_color(request):
    if request.method == 'POST':
        try:
            ...
            cell.increment_color_count(color)
