...
class SigComponent extends React.Component {
  ...
  async fetchSig() {
    ...
    await axios.get(`http://127.0.0.1:8000/sig/${this.props.location.state.sigName}/`).then((res) => {
      this.setState({
        sigData: res.data,
      });
    });
    ...
    await axios.get(`http://127.0.0.1:8000/project/current/${this.props.location.state.sigName}/`).then((res) => {
      this.setState({
        ...
        <div className="sigComponent">
          ...
          <div className="row">
            <div className="col l10 offset-l1 s10 offset-s1">
              <h4 className="center">
                <span>Projects {this.printYear(this.state.projectData[0].year)}</span>
              </h4>
            </div>
          </div>
